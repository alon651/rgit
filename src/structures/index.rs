...
impl Index{
    ...
    pub fn read(path: &std::path::Path) -> anyhow::Result<Self>{
        ...
        let entries: Vec<IndexEntry> = Vec::with_capacity(count as usize);

        let mut idx = 12;
        for _ in 0..count{
            println!("index: {}", idx);
            let ctime = u32::from_be_bytes(buf[idx..idx+4].try_into()?);
            let ctime_nsec = u32::from_be_bytes(buf[idx+4..idx+8].try_into()?);
            let mtime = u32::from_be_bytes(buf[idx+8..idx+12].try_into()?);
            let mtime_nsec = u32::from_be_bytes(buf[idx+12..idx+16].try_into()?);
            ...
            let mode_type = match mode_type {
                ...
                h => return Err(anyhow!("invalid mode {:b}", h)),
            };
            println!("mode type: {:?}, perms: {:o}", mode_type, perms);
            ...
            let sha1 = encode(&buf[idx+40..idx+60]);
            println!("sha1: {}", sha1);
            ...
            ensure!(buf[idx+(name_len as usize)] == 0, "invalid index entry name");
            println!("hello:c {:?}",&buf[idx+(name_len as usize)]);
            idx += name_len as usize + 1;
            while idx % 8 != 0{
                idx += 1;
            }
            
            println!("name: {}, assume_valid: {}, stage: {}", name, assume_valid, stage);

        }

        Err(anyhow!("index version: {}, count: {}", version, count))
    }
}